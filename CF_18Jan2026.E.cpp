...
signed main(void){
    ...
    while(T--){
        ...
        sort(b.begin(), b.end()); 
        string s; cin>>s; 
        vector<int> left(n, 1e15), right(n, 1e15); 
        for(int i = 0; i < n; i++){
            auto ub = upper_bound(b.begin(), b.end(), arr[i]); 
            if(ub != b.end()){
                right[i] = (b[ub-b.begin()] - arr[i]); 
            } 
            auto lb = lower_bound(b.begin(), b.end(), arr[i]); 
            int index = lb - b.begin(); 
            if((index-1) >= 0){
                left[i] = arr[i] - b[index-1];  
            }
        } 
        sort(left.begin(), left.end()); sort(right.begin(), right.end()); 
        int pref = 0, count = n; // count means no. of robots alive.  
        for(int i = 0; i < k; i++){
            pref += (s[i] == 'R' ? 1 : -1); 
            if(pref > 0){
                auto ub = upper_bound(right.begin(), right.end(), pref); 
                int index = ub - right.begin(); 
                count = min(count, (n-index)); 
                cout<<count<<" ";  
            } 
            else if(pref < 0){
                auto ub = upper_bound(left.begin(), left.end(), (-1*pref)); 
